<template>
    <div>
        <div class="flex">
            <a :href="'/projects/clients/'+this.client.id">
                <svg
                    class="w-min"
            ...
            <div class="dark:text-gray-400 text-sm">Client</div>
        </div>
        <form action="#" @submit.prevent="saveClient()" class="space-y-4">
            <div class="flex justify-between">
                ...
                <div class="space-x-3">
                    <a
                        :href="'/projects/clients/'+this.client.id"
                        class="inline-flex items-center px-8 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center text-white rounded dark:border-2 dark:border-white"
                    >
                        Cancel
                    </a>
                    <button
                        ...
                        class="inline-flex items-center px-8 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center text-white bg-primary-700 dark:border-2 border-transparent rounded hover:bg-primary-800 dark:bg-brandPrimary"
                    >
                        Save
                    </button>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="border-t border-gray-600 ">
                    ...
                    <div id="client_information" class="grid grid-cols-2 gap-10">
                        <div>
                            <input
                                type="text"
                                name="company_name"
                ...
                <div class="border-t border-gray-600 ">
                    <div class="grid grid-cols-2 gap-10">
                        <div class="space-y-4">
                            <label for="tag" class="block text-lg font-medium dark:text-white my-5">Customize Tag</label>
                            <input
                                type="text"
                                name="tag"
                                v-model="client.tag"
                            ...
                            <div class="space-x-2">
                                <input type="color" name="tag_bg_color" id="tag_bg_color" v-model="client.tag_bg_color">
...
export default {
    mounted() {
        ...
        if(this.client_data){
            const data = JSON.parse(this.client_data);
            this.client.id = data.id;
            this.client.company_name = data.company_name;
            this.client.email = data.email;
            this.client.key_contact = data.key_contact;
            this.client.phone_number = data.phone_number;
            this.client.tag = data.tag;
            this.client.tag_bg_color = data.tag_bg_color;
            this.client.tag_color = data.tag_color;

            if(data.company_logo){
                this.company_logo_preview =  '/storage/upload/projects/clients/'+data.company_logo;
            }
        }
    },
    data() {
    ...
    methods: {
        saveClient() {
            ...
            axios
                .post("/projects/clients/update", formData)
                .then((response) => {
                    setTimeout(function () {
