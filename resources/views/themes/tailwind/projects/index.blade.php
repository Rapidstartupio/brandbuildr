    ...
    <div class="text-sm font-medium text-center text-gray-500 dark:text-gray-400 md:flex">
        ...
        <div class="text-left md:flex-auto mt-5 md:mt-0">
            <a href="{{route('projects.create')}}">
                <button type="button" class=" md:float-right text-white bg-blue-700  hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-wave-500 dark:hover:bg-wave-500 focus:outline-none dark:focus:ring-blue-800"><span class="md:px-4 font-bold">+</span> New Project</button>
            </a>
        </div>
    </div>
</div>
...
<div id="filters" class="lg:flex space-y-2 lg:space-y-0">
    <div class="client mr-4">
        <div class="relative max-w-sm">
            ...
            <select id="project-client" class="w-full text-gray-900 text-sm rounded  block  pl-10 p-2.5   dark:placeholder-gray-400 dark:text-white  border-0 brandDark2" onchange="filterAndNavigate()">
                <option class="text-black" selected value="">Client</option>
                @if(auth()->user()->clients)
                @foreach(auth()->user()->clients as $clt)
                <option class="text-black" value="{{$clt->id}}" @if($clt->id == $filter->client) selected @endif>{{$clt->company_name}}</option>
                @endforeach
                @endif
            </select>
    ...
    <div class="dates mr-4">
        <div class="relative max-w-sm">
            ...
            <input datepicker id="project-deadline" value="{{$filter->deadline}}" onchange="filterAndNavigate()" type="date" class=" w-full text-gray-900 text-sm rounded  block  pl-4 p-2.5   dark:placeholder-gray-400 dark:text-white  border-0 brandDark2" placeholder="Date">
        </div>
    </div>
</div>
...
<div class="flex justify-center text-white text-center md:mt-10">
    <div class="max-w-xl space-y-2 md:space-y-8">
        ...
        <div class="">
            <a href="{{route('projects.create')}}">
                <button type="button" class="text-white bg-wave-500 hover:bg-wave-600 font-medium rounded-lg text-lg px-10 py-4 dark:bg-brandPrimary dark:hover:bg-wave-500 focus:outline-none dark:focus:ring-blue-800">Create New Project</button>
            </a>
        </div>
    </div>
</div>
